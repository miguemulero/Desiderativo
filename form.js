...
${protocolo}`;
  ...
  async function startAnalysis(protocoloText) {
    ...
    setTimeout(() => {
      window.open('https://notebooklm.google.com/?desiderativo=true', 'notebooklm_window');
    }, 1000);
    ...
    return new Promise((resolve) => {
        ...
        if (responseStr) {
          try {
            ...
            if (response.id === analysisId && response.status === 'completed') {
              ...
              resolve({ ok: true, reportText: response.reportText });
            }
          } catch (e) {
      ...
      setTimeout(() => {
        clearInterval(checkInterval);
        resolve({ ok: false, error: "Tiempo agotado" });
      }, 300000);
    });
  }
...
// Listener para detectar respuestas de NotebookLM
  ...
  if (e.key === 'desiderativo_response' && e.newValue) {
    try {
      ...
      if (response.id === currentId && response.status === 'completed') {
        ...
        resultText.value = response.reportText;
        resultSection.style.display = "block";
        resultSection.scrollIntoView({ behavior: "smooth", block: "start" });
